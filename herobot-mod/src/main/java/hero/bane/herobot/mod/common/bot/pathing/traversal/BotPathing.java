package hero.bane.herobot.mod.common.bot.pathing.traversal;
...
public class BotPathing {
    ...
    private void finish(boolean reached) {
        if (done) return;
        if (reached) hero.bane.herobot.mod.common.ai.AiScriptRegistry.markPathReached(bot);
        else hero.bane.herobot.mod.common.ai.AiScriptRegistry.markPathFailed(bot);
        stop();
    }
