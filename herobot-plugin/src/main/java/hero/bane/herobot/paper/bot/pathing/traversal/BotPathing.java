package hero.bane.herobot.paper.bot.pathing.traversal;
...
import hero.bane.herobot.paper.bot.pathing.placement.MovementHelper;
import hero.bane.herobot.paper.bot.pathing.placement.PathFinder;
...
public class BotPathing {
    ...
    private void finish(boolean reached) {
        if (done) return;
        if (reached) hero.bane.herobot.paper.ai.AiScriptRegistry.markPathReached(bot);
        else hero.bane.herobot.paper.ai.AiScriptRegistry.markPathFailed(bot);
        stop();
    }
