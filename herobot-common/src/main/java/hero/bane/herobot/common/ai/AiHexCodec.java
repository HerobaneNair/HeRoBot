...
public final class AiHexCodec {
    ...
    public static String encode(AiScript script) {
        byte[] json = AiScriptCodec.toJson(script).getBytes(StandardCharsets.UTF_8);
    ...
    public static AiScript decode(String hex, String fallbackName) {
        ...
        byte[] json = inflate(framed, framed.length - 1);
        try {
            return AiScriptCodec.fromJson(new String(json, StandardCharsets.UTF_8), fallbackName);
    ...
    private static byte[] inflate(byte[] data, int length) {
        Inflater inflater = new Inflater(true);
        try {
            inflater.setInput(data, 1, length);
