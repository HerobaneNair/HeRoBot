...
public final class BlockDescriptions {
    ...
    static {
        ...
        DESCRIPTIONS.put(BlockType.PLAY_SOUND, "Play a sound file from the world's herobot_sounds folder out of the bot's mouth over Simple Voice Chat (.wav or .mp3, file name including the extension). Loop replays it until stopped. Starting a sound stops whatever the bot was saying before");
        DESCRIPTIONS.put(BlockType.STOP_SOUND, "Stop the sound the bot is currently playing");
        DESCRIPTIONS.put(BlockType.IS_SPEAKING, "True while the bot is playing a sound or relaying a bluetooth source over voice chat");
        DESCRIPTIONS.put(BlockType.BLUETOOTH, "Use the bot as a bluetooth speaker for a player: everything that player says into their mic comes back out of the bot. Stops any sound the bot was playing");
        DESCRIPTIONS.put(BlockType.STOP_BLUETOOTH, "Stop relaying a player's microphone through the bot");
        DESCRIPTIONS.put(BlockType.IS_BLUETOOTHED, "True while the bot is set up as a bluetooth speaker for some player");
